...
// Lokalna definicija tipa za CyberIncidentReport
interface CyberIncidentReport {
  ...
  naziv_subjekta?: string
  sektor?: string
  kontakt_tehnicni_ime?: string
  kontakt_tehnicni_email?: string
  kontakt_tehnicni_telefon?: string
  kontakt_oseba_ime?: string
  kontakt_oseba_email?: string
  kontakt_oseba_telefon?: string
  // Sekcija 2: Začetne informacije
  zacetek_incidenta?: string
  cas_zaznave?: string
  opis_incidenta?: string
  taksonomija?: string
  ocena_nevarnosti?: string
  ocena_vpliva?: string
  stopnja_incidenta?: string
  opombe?: string
  // Sekcija 3: Vmesno/končno poročanje
  cas_zadnjega_porocanja?: string
  ...
  izvor_email?: boolean
  izvor_vdor?: boolean
  izvor_spletno?: boolean
  izvor_datoteke?: boolean
  izvor_drugo?: string
  ogrozena_storitev_zinfv?: boolean
  ogrozena_storitev_ostale?: boolean
  cezmejni_vpliv_da_ne?: boolean
  cezmejni_vpliv_opis?: string | null
  akcijski_ze_sprejeti?: string
  akcijski_nacrtovani?: string
  povzrocena_skoda?: string
  potrebe_odprava?: string
  casovni_okvir?: string
  priloge_seznam?: string
...
export default function CyberIncidentReportPage() {
  ...
  const handleSendReportToSICERT = async (report: CyberIncidentReport) => {
    ...
    try {
      // Step 1: Generate PDF
      const { data: pdfData, error: pdfError } = await mysqlAPI.functions.invoke('generate-incident-report-pdf', {
...
${report.kontakt_oseba_ime || report.contact_name || ''}`)
  ...
  const handleExportPDF = async (report: CyberIncidentReport) => {
    ...
    try {
      const { data, error } = await mysqlAPI.functions.invoke('generate-incident-report-pdf', {
        body: { reportId: report.id }
      })
