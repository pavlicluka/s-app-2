...
export default function FunctionLogsViewer({
  ...
  async function fetchLogs() {
    ...
    setError(null)
    try {
      // MySQL ne potrebuje session za dostop do logov
      // const { data: { session } } = await supabase.auth.getSession()
      // if (!session?.access_token) {
      //   throw new Error('No active session')
      // }

      // MySQL funkcija za pridobivanje logov
      const { data, error: invokeError } = await mysqlAPI.query(
        'SELECT * FROM function_logs WHERE function_name = ? AND created_at >= DATE_SUB(NOW(), INTERVAL ? HOUR) ORDER BY created_at DESC LIMIT ?',
        [functionName, hoursBack, limit]
      )
