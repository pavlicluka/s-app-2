...
export default function InventoryExpiringItems() {
  ...
  const fetchExpiringItems = async () => {
    try {
      ...
      const { data: licenses, error: licensesError } = await mysqlAPI.select('inventory_licenses', '*', 'expiry_date IS NOT NULL', [], { orderBy: 'expiry_date', ascending: true })
      ...
      const { data: software, error: softwareError } = await mysqlAPI.select('inventory_software', '*', '', [])
