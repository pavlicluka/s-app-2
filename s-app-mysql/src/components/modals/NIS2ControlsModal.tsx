...
// Lokalno definiran NIS2Control tip
...
export default function NIS2ControlsModal({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (control) {
        const result = await mysqlAPI.update('nis2_controls', controlData, { id: control.id })
        if (result.error) throw result.error
      } else {
        const result = await mysqlAPI.insert('nis2_controls', controlData)
        if (result.error) throw result.error
      }
