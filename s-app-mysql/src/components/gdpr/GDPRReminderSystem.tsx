...
export function GDPRReminderSystem({ className = '' }: ReminderSystemProps) {
  ...
  const fetchReminderAlerts = async () => {
    try {
      ...
      const { data: incidents, error } = await mysqlAPI
        .select('gdpr_data_breach_log', `
          breach_id,
          urgency_status,
          breach_date,
          severity,
          ...
          created_at,
          updated_at
        `, '', [], { orderBy: 'breach_date', ascending: false })
        
      // Filter incidents for urgency status and status
      const filteredIncidents = (incidents || []).filter(incident => 
        ['warning', 'urgent', 'overdue'].includes(incident.urgency_status) && 
      ...
      const alertsWithTime = filteredIncidents.map(incident => {
