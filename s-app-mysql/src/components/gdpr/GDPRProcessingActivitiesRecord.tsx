...
export default function GDPRProcessingActivitiesRecord() {
  ...
  const fetchRecords = async () => {
    try {
      const { data, error } = await mysqlAPI
        .select('gdpr_processing_activities_record', '*', '', [], { orderBy: 'created_at', ascending: false })
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const { error } = await mysqlAPI.insert('gdpr_processing_activities_record', formData)
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const { error } = await mysqlAPI.update('gdpr_processing_activities_record', formData, 'id = ?', [selectedRecord.id])
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const { error } = await mysqlAPI.delete('gdpr_processing_activities_record', 'id = ?', [selectedRecord.id])
