...
export default function GDPRDataBreachLogAddModal({ isOpen, onClose, onSave }: GDPRDataBreachLogAddModalProps) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert([{
          ...
          affected_records: parseInt(formData.affected_records) || 0,
          reported_to_authority: formData.reported_to_authority === 'true',
          ...(fileData && {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-surface border border-border-subtle rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-subtle">
          <h2 className="text-h3 text-text-primary">{t('modals.add.gdprDataBreachLog.title')}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">
                {t('gdpr.breach.breachId')} <span className="text-status-error">*</span>
              </label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">
                {t('gdpr.breach.severity')} <span className="text-status-error">*</span>
              </label>
              <select
                required
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleChange('breach_type', e.target.value)}
                className="w-full px-3 py-2 bg-bg-surface border border-border-subtle rounded text-text-primary focus:border-accent-primary focus:outline-none"
              >
            ...
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">
                {t('gdpr.breach.reportedYes')}
              </label>
              <select
                value={formData.reported_to_authority}
                onChange={(e) => handleChange('reported_to_authority', e.target.value)}
                className="w-full px-3 py-2 bg-bg-surface border border-border-subtle rounded text-text-primary focus:border-accent-primary focus:outline-none"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleChange('reporting_date', e.target.value)}
                className="w-full px-3 py-2 bg-bg-surface border border-border-subtle rounded text-text-primary focus:border-accent-primary focus:outline-none"
              />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                onChange={(e) => handleChange('mitigation_steps', e.target.value)}
